...
```
用户：/auto npm run build 报错了，类型不匹配
...
```
用户：/auto README 需要更新，加上新的 API 说明
...
### 模糊输入
当用户输入过于模糊时（如 `/auto 帮帮我`），默认选择 `/plan` 并提示用户提供更多信息。

### 多意图输入
当用户输入包含多个意图时（如 `/auto 修复构建错误然后跑一下测试`），优先处理高优先级任务，完成后提示用户执行下一个任务。
...
| `/build-fix` | 构建错误与类型问题修复 |
| `/e2e` | 端到端测试生成与执行 |
| `/test-coverage` | 测试覆盖率分析与提升 |
| `/refactor-clean` | 代码重构与死代码清理 |
| `/update-docs` | 文档更新与维护 |
| `/update-codemaps` | 代码架构图更新 |
